		...
		$googleMapInit = "
			...
			function bindInfoBox(marker, map, infoWindow, html) {
				...
				google.maps.event.addListener(marker, 'mouseover', function() {
					...
					$(\"#poiid-\"+marker.id).addClass(\"imc-highlight\");
				});			  
				
				google.maps.event.addListener(marker, 'mouseout', function() {
					infoBox.close();
					$(\"#poiid-\"+marker.id).removeClass(\"imc-highlight\");
				});			  
			}			
			...
			window.addEvent('domready', function() {
				$(\".imc-poi-item\").mouseenter(function(event)
				{
					$(this).addClass(\"imc-highlight\");
					markerhover($(this).attr('id').substring(8));
				});

				$(\".imc-poi-item\").mouseleave(function(event)
				{
