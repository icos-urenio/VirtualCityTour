 ...
 * Model
 */
class VirtualcitytourModelPois extends JModelList
{
	//protected $_item;
	private $_categories = null;
	private $_parent = null;
	private $_params = null;
	//public $_context = 'com_virtualcitytour.pois';	
	...
	{
        $_categories = JCategories::getInstance('Virtualcitytour');
	...
	function getItems()
	{
		...
		//so I get model (discussions) from within another model (pois) ...
		
		//JModel::addIncludePath(JPATH_SITE.'/components/com_virtualcitytour/models', 'Discussions'); //don't need this we are already inside a model ;)
		...
		for ($i = 0, $n = count($items); $i < $n; $i++) {
			...
			$item->reported_rel = VirtualcitytourHelper::getRelativeTime($item->reported);
			$item->acknowledged_rel = VirtualcitytourHelper::getRelativeTime($item->acknowledged);
			$item->closed_rel = VirtualcitytourHelper::getRelativeTime($item->closed);
	...
	function getTimestamp()
	{
		...
		$query->select('a.*');
		$query->from('`#__virtualcitytour_timestamp` AS a');
		$query->where('a.id = 1');
	...
	function getSimpleCategories()
	{
		...
		$query->where('c.extension = "com_virtualcitytour"');
