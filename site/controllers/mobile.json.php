...
class VirtualcitytourControllerMobile extends JController
{
	...
	/* arguments: 
	 * limit=0 : get ALL pois, limit=5 get recent 5 pois
	 * showComments=1: includes poi's discussion, showComments=0 (default) discussion is not included
	 * x0up: longitude < x0up
	...
	* */	
	public function getPoi()
	{
		//get request
		$showComments = JRequest::getInt('showComments');
		$poiId = JRequest::getInt('poiId');
	
		//get model and items
		$model = $this->getModel('poi');

		$item = $model->getItem($poiId);
		if($item == null){			
			echo json_encode('PoiId: ' .$poiId.' not found');
			return;
		}
	...
	public function votePoi()
	{
		...
		//check if user has already voted for the poi
		$hasVoted = $model->getHasVoted($poiId, $userid);
		if($hasVoted){
			echo json_encode("-1");
			return;
		}	
		...
		//do the voting
		$newVotes = $model->vote($poiId, $userid);		
	...
	public function addComment()
	{
		...
		$lastComment = $model->comment($poiId, $userid, $description);
